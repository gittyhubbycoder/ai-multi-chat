    ...
    >
      {messages.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full text-center p-4 sm:p-8">
          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center mb-4 sm:mb-6">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-indigo-400 w-10 h-10 sm:w-12 sm:h-12">
          ...
          <h2 className="text-xl sm:text-2xl font-semibold mb-2 px-4">Start a conversation</h2>
          ...
          <div className="flex flex-wrap gap-2 mt-4 sm:mt-6 justify-center px-4">
            {['Explain quantum computing', 'Write a poem about AI', 'Help me with Python code'].map(prompt => (
              <button
                key={prompt}
                className="glass-button-secondary px-3 sm:px-4 py-2 text-xs sm:text-sm rounded-full"
              >
                {prompt}
              </button>
            ))}
