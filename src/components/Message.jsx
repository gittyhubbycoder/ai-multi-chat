...
export default function Message({ message, modelColor }) {
  ...
  return (
    <div className={`flex gap-3 sm:gap-4 p-4 sm:p-6 animate-fadeIn ${isUser ? '' : 'bg-white/5'}`}>
      <div 
        className={`
      ...
      >
        {isUser ? (
          ...
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4 sm:w-5 sm:h-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        {message.file && (
          <div className="mb-3 p-2 sm:p-2.5 bg-white/10 rounded-lg inline-flex items-center gap-2 text-sm">
            <span>📎</span>
        ...
        {isUser ? (
          <p className="text-white/90 whitespace-pre-wrap text-sm sm:text-base leading-relaxed">{message.content}</p>
        ) : (
          <div 
            className="markdown-content text-white/90 text-sm sm:text-base"
        ...
        {!isUser && (
          <button
            onClick={handleCopy}
            className="mt-3 sm:mt-4 flex items-center gap-1.5 text-xs text-white/40 hover:text-white/70 transition-colors"
          >
